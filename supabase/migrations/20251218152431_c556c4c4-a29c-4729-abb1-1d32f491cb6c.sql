-- Insert new OWASP step for A10 REST Documentation Endpoint Check
-- This step checks if /rest-doc/ is publicly accessible (runs before manual verification)

INSERT INTO public.owasp_steps (user_id, owasp_item_id, step_name, step_description, edge_function_name, step_order, is_active, needs_domain_model, needs_railway_analysis)
SELECT 
  oi.user_id,
  oi.id as owasp_item_id,
  'Check REST Documentation Endpoint' as step_name,
  'Verifies that the /rest-doc/ endpoint is not publicly accessible to prevent SSRF reconnaissance' as step_description,
  'owasp-check-a10-endpoint-restdoc' as edge_function_name,
  0 as step_order,
  true as is_active,
  false as needs_domain_model,
  false as needs_railway_analysis
FROM public.owasp_items oi
WHERE oi.owasp_id = 'A10'
ON CONFLICT DO NOTHING;

-- Update existing A10 manual verification step to have step_order = 1 (if it exists)
UPDATE public.owasp_steps 
SET step_order = 1
WHERE edge_function_name = 'owasp-check-a10-manual-verification';

-- Update initialize_edge_functions to include the new function
...
BEGIN
  ...
  VALUES
    ...
    (target_user_id, 'owasp-check-a10-manual-verification', 'A10 Manual URL Verification', 'Manual verification for Server-Side Request Forgery (SSRF) prevention', 'owasp', true),
    (target_user_id, 'owasp-check-a10-endpoint-restdoc', 'A10 REST Doc Endpoint Check', 'Checks if /rest-doc/ endpoint is publicly accessible for SSRF reconnaissance', 'owasp', true),
