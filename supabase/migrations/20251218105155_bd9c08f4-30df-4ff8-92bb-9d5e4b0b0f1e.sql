-- Create table for JavaScript whitelist entries
CREATE TABLE public.owasp_js_whitelist (
  ...
  UNIQUE(user_id, app_id, script_pattern)
);

-- Enable RLS
ALTER TABLE public.owasp_js_whitelist ENABLE ROW LEVEL SECURITY;

-- RLS Policies
CREATE POLICY "Users can view their own whitelist entries"
ON public.owasp_js_whitelist
FOR SELECT
USING (auth.uid() = user_id);

CREATE POLICY "Users can create their own whitelist entries"
ON public.owasp_js_whitelist
FOR INSERT
WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own whitelist entries"
ON public.owasp_js_whitelist
FOR UPDATE
USING (auth.uid() = user_id);

CREATE POLICY "Users can delete their own whitelist entries"
ON public.owasp_js_whitelist
FOR DELETE
USING (auth.uid() = user_id);

-- Add trigger for updated_at
CREATE TRIGGER update_owasp_js_whitelist_updated_at
BEFORE UPDATE ON public.owasp_js_whitelist
FOR EACH ROW
EXECUTE FUNCTION public.update_updated_at_column();

-- Update initialize_edge_functions to include A05 check
