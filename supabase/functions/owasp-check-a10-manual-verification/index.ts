...
serve(async (req) => {
  ...
  try {
    ...
    console.log('[A10-Manual-Verification] Request:', { 
      user_id, 
      app_id, 
      environment_name, 
      owasp_item_id 
    ...
    if (owaspItemError || !owaspItem) {
      console.error('[A10-Manual-Verification] OWASP item not found:', owaspItemError);
      return new Response(
    ...
    if (urlsError) {
      console.error('[A10-Manual-Verification] Error fetching URLs:', urlsError);
      return new Response(
    ...
    if (!urls || urls.length === 0) {
      console.log('[A10-Manual-Verification] No URLs configured');
      return new Response(
        JSON.stringify({
          status: 'warning',
          details: 'No verification URLs configured. Add URLs in OWASP Settings to enable SSRF prevention verification.',
          urls: [],
          verification: null,
        }),
    ...
    if (verificationError || !verification) {
      console.log('[A10-Manual-Verification] Never verified');
      return new Response(
        JSON.stringify({
          status: 'fail',
          details: `SSRF prevention URLs have never been verified for this environment. Please review the ${urls.length} configured URL(s) and mark as verified.`,
    ...
    if (now > expirationDate) {
      console.log('[A10-Manual-Verification] Verification expired:', {
      ...
      return new Response(
        JSON.stringify({
          status: 'fail',
          details: `Verification expired on ${expirationDate.toLocaleDateString()}. Last verified: ${verifiedAt.toLocaleDateString()}. Please re-verify the ${urls.length} configured URL(s) for SSRF prevention.`,
          urls: urls as ManualCheckUrl[],
    ...
    console.log('[A10-Manual-Verification] Verification valid:', {
      verified_at: verification.verified_at,
      expiration_date: expirationDate.toISOString(),
    });
    return new Response(
      JSON.stringify({
        status: 'pass',
        details: `Last verified: ${verifiedAt.toLocaleDateString()}. Valid until: ${expirationDate.toLocaleDateString()}. ${urls.length} URL(s) configured for SSRF prevention.`,
        urls: urls as ManualCheckUrl[],
        verification: verification as ManualVerification,
        expiration_months: owaspItem.expiration_months,
        expiration_date: expirationDate.toISOString(),
    ...
    console.error('[A10-Manual-Verification] Unexpected error:', error);
    return new Response(
      JSON.stringify({ 
        status: 'error', 
