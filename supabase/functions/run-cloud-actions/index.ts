    ...
    try {
      ...
      switch (action.action_type) {
        ...
        case "transport":
          ...
          // Step 3: Stop target environment
          await supabase.from("cloud_action_logs").insert({
            user_id: user.id,
            action_id: action.id,
            level: "info",
            message: `Step 3: Stopping target environment ${normalizedEnvironmentName}`,
          });
          ...
          // Step 4: Create backup of target environment (reuse deploy logic)
          await supabase.from("cloud_action_logs").insert({
            user_id: user.id,
            action_id: action.id,
            level: "info",
          ...
          const transportBackupUrl = `https://deploy.mendix.com/api/2/apps/${encodeURIComponent(transportTargetProjectId)}/environments/${encodeURIComponent(transportEnvironmentId)}/snapshots`;
          ...
          await supabase.from("cloud_action_logs").insert({
            ...
            action_id: action.id,
            level: "info",
            message: `Step 5: Polling for backup completion`,
          });
          ...
          // Step 6: Start target environment
          await supabase.from("cloud_action_logs").insert({
            user_id: user.id,
            action_id: action.id,
            level: "info",
            message: `Step 6: Starting target environment ${normalizedEnvironmentName}`,
          });
