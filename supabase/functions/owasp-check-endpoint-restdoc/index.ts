...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`[OWASP A01] Checking /rest-doc/ endpoint for project: ${project_id}, environment: ${environment_name}`);
    ...
    try {
      const response = await fetch(testUrl, {
        ...
        // Don't follow redirects to get the actual response
        redirect: 'manual',
      });

      httpStatus = response.status;
      statusText = response.statusText;
      console.log(`Endpoint responded with status: ${httpStatus} ${statusText}`);
    } catch (fetchError) {
      console.log('Fetch error (likely network issue):', fetchError);
    ...
    // 2xx = FAIL (endpoint is publicly accessible - security issue)
    // 4xx or 5xx = PASS (endpoint is protected)
    if (httpStatus >= 200 && httpStatus < 300) {
      return new Response(
        JSON.stringify({
          status: 'fail',
          details: `✗ SECURITY ISSUE: REST documentation endpoint is publicly accessible (HTTP ${httpStatus}). The /rest-doc/ endpoint should not be accessible to the public.`,
        }),
        {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        }
      ...
      return new Response(
        JSON.stringify({
          status: 'warning',
          details: `⚠ REST documentation endpoint returned a redirect (HTTP ${httpStatus}). Manual verification recommended.`,
        }),
        {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        }
    ...
    console.error('Error in owasp-check-endpoint-restdoc:', error);
    return new Response(
      JSON.stringify({
        status: 'error',
