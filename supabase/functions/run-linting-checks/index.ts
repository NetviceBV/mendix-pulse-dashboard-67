...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 4. Try Git first, fallback to SVN
    ...
    if (!analyzerResponse) {
      ...
      if (!svnRes.ok) {
        ...
        return new Response(JSON.stringify({ error: `Analyzer API failed: ${errText}` }), {
    ...
    if (!mxlint) {
      ...
      return new Response(JSON.stringify({ error: 'Invalid analyzer response: missing mxlint data' }), {
        status: 502, headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
    ...
    mxlint.violations?.forEach((v: any) => {
      // Extract rule number from the path (e.g. 001_0002 from the path)
    ...
    const resultRows = (mxlint.rules || []).map((rule: any) => {
      ...
      // Extract chapter from path directory prefix (e.g. "001_project_settings")
    ...
    return new Response(JSON.stringify({
      ...
      summary: {
        ...
        total: summary.total ?? resultRows.length,
      },
    }), {
