...
serve(async (req) => {
  ...
  try {
    ...
    console.log('[A03 Manual Verification Check] Request:', { 
      user_id, 
      app_id, 
      environment_name, 
      owasp_item_id 
    ...
    if (owaspItemError || !owaspItem) {
      console.error('[A03 Manual Verification Check] OWASP item not found:', owaspItemError);
      return new Response(
    ...
    if (urlsError) {
      console.error('[A03 Manual Verification Check] Error fetching URLs:', urlsError);
      return new Response(
    ...
    if (!urls || urls.length === 0) {
      console.log('[A03 Manual Verification Check] No URLs configured');
      return new Response(
        JSON.stringify({
          status: 'warning',
    ...
    if (verificationError || !verification) {
      console.log('[A03 Manual Verification Check] Never verified');
      return new Response(
        JSON.stringify({
          status: 'fail',
    ...
    if (now > expirationDate) {
      console.log('[A03 Manual Verification Check] Verification expired:', {
    ...
    console.log('[A03 Manual Verification Check] Verification valid:', {
      verified_at: verification.verified_at,
      expiration_date: expirationDate.toISOString(),
    });
    return new Response(
    ...
    console.error('[A03 Manual Verification Check] Unexpected error:', error);
    return new Response(
      JSON.stringify({ 
        status: 'error', 
